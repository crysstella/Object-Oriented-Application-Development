...
public class CheckInput {
	...
	/**
	 * Takes in a string with only letters from the user.
	 * @return the inputted String.
	 */
  /*public static String getName(){
    ...
    while (!valid){
      ...
      for (int i=0; i< input.length(); i++){
        if (((Character.isLetter(input.charAt(i))) == false) || ((Character.isSpace(input.charAt(i))) == false)){
          valid = false;
        }
      }
      if (!valid){
        System.out.println("Name should not have special character and numbers!");
      }
    }
